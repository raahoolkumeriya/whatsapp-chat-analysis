...
class WhatsAppChatAnalysis:
    ...
    def generate_word_cloud(self, text):
        ...
        stopwords = set(STOPWORDS).union(
            set(self.add_multilingual_stopwords()))
    ...
    #     image = fig.update_traces(
    #       textposition='inside', textinfo='percent+label')
    #     return image

    def get_user_json_data(
            self, user_number, dataframe,
            media_messages_df, author, wordcloud_string):
        subdict = dict()
        ...
        subdict["avg_words_per_msg"] = '{:.2f}'.format(
            (np.sum(dataframe['Word_Count']))/dataframe.shape[0])
        subdict["media_msg"] = \
            media_messages_df[media_messages_df['Author'] == author].shape[0]
    ...
    def convert_raw_to_dataframe_data(self, exported_chat_file):
        ...
        new_df = new_df.assign(
            Emojis=new_df["Message"].apply(self.extract_emojis))
        # Handle Links
        new_df = new_df.assign(
            Urlcount=new_df["Message"].apply(
                lambda x: re.findall(self.URLPATTERN, x)).str.len())
        # Handle Image ommited
        new_df = new_df.assign(
            Media=new_df['Message'].apply(
                lambda x: re.findall("image omitted", x)).str.len())
        # Handling wordCount
        media_messages_df = \
            new_df[new_df['Message'].str.contains("image omitted")]
